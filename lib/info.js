...
const fixName = (name, appName, appSlug) => {
  return String(name).replace(/electron/i, match => {
    const isLowerCase = match === match.toLowerCase()
    return isLowerCase ? appSlug : appName
  })
}

const rename = async (paths, appName, appSlug) => {
  ...
  for (const file of files) {
    ...
    const newName = fixName(details.name, appName, appSlug)
...
const setDetails = async (files, appName, appSlug) => {
  ...
  for (const file of files) {
    ...
    Object.keys(parsed).forEach((key, index) => {
      parsed[key] = fixName(parsed[key], appName, appSlug)
    })
