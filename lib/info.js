...
const setDetails = async (paths, appName, lowerAppName) => {
  for (const path of paths) {
    ...
    await fs.writeFile(path, plist.build(parsed))
  }
}

const renameFiles = async (paths, appName, lowerAppName) => {
  const oldPaths = sortByLevels(paths)
...
module.exports = async (output, appName) => {
  ...
  await renameFiles(toRename, appName, lowerAppName)
}
